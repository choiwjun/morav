...
export async function searchUnsplashImages(
  ...
  if (!accessKey) {
    ...
    return searchPexelsImages(query, perPage);
  }
...
async function searchPexelsImages(
  ...
  if (!apiKey) {
    ...
    return getRandomPicsumImage(query);
  }
...
/**
 * Lorem Picsum에서 랜덤 이미지 (최종 백업)
 * API 키 불필요
 */
function getRandomPicsumImage(alt: string): UnsplashSearchResult {
  // 랜덤 seed 생성
...
/**
 * 키워드를 영문 검색어로 변환
 * 한글 키워드를 영문으로 번역하는 간단한 매핑
 */
export function translateKeywordToEnglish(keyword: string): string {
  // 이미 영문이면 그대로 반환
...
export function extractImageSearchQuery(
  ...
  // 섹션 제목에서 핵심 키워드 추출
  if (sectionTitle) {
    const translated = translateKeywordToEnglish(sectionTitle);
