import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@/lib/supabase/server';
import { encrypt } from '@/lib/crypto';

export async function POST(request: NextRequest) {
  try {
    const supabase = await createClient();
    const {
      data: { user },
    } = await supabase.auth.getUser();

    if (!user) {
      return NextResponse.json({ error: '로그인이 필요합니다.' }, { status: 401 });
    }

    const body = await request.json();
    const { blogName, blogUrl, accessToken } = body;

    if (!blogName || !blogUrl || !accessToken) {
      return NextResponse.json(
        { error: '블로그 이름, 주소, Access Token을 모두 입력해주세요.' },
        { status: 400 }
      );
    }

    // Access Token 암호화
    const encryptedToken = encrypt(accessToken);

    // 블로그 URL 정규화
    const normalizedUrl = blogUrl.startsWith('http') ? blogUrl : `https://${blogUrl}`;

    // 블로그 저장 (upsert)
    const { data: savedBlog, error: upsertError } = await supabase
      .from('blogs')
      .upsert(
        {
          user_id: user.id,
          platform: 'tistory',
          blog_name: blogName.slice(0, 100),
          blog_url: normalizedUrl.slice(0, 255),
          access_token: encryptedToken,
          is_active: true,
        },
        {
          onConflict: 'user_id,platform,blog_url',
        }
      )
      .select()
      .single();

    if (upsertError) {
      console.error('Blog upsert error:', upsertError);
      return NextResponse.json({ error: '블로그 정보 저장에 실패했습니다.' }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      blog: {
        id: savedBlog.id,
        platform: savedBlog.platform,
        blog_name: savedBlog.blog_name,
        blog_url: savedBlog.blog_url,
      },
    });
  } catch (error) {
    console.error('Tistory connect error:', error);
    return NextResponse.json({ error: '티스토리 연결 중 오류가 발생했습니다.' }, { status: 500 });
  }
}
